...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom frankohash engine.
	frankohash := NewTester([]string{server.URL}, false)
	...
	frankohash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := frankohash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	frankohash := New(config, []string{server.URL}, false)
	defer frankohash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom frankohash engine.
	frankohash := NewTester([]string{server.URL}, false)
	frankohash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer frankohash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			frankohash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, frankohash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch frankohash result timeout", id+1)
		}
	}
}
