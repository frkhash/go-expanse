...
var errEthashStopped = errors.New("frkhash stopped")
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.frkhash.remote.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashrate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.frkhash.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.frkhash.remote.exitCh:
		return false
	}
