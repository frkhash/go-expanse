...
func (frkhash *Frkhash) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	...
	return frkhash.verifyHeader(chain, header, parent, false, seal, time.Now().Unix())
}
...
func (frkhash *Frkhash) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if frkhash.config.PowMode == ModeFullFake || len(headers) == 0 {
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = frkhash.verifyHeaderWorker(chain, headers, seals, index, unixNow)
				done <- index
			}
		}()
	}
...
func (frkhash *Frkhash) verifyHeaderWorker(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool, index int, unixNow int64) error {
	...
	return frkhash.verifyHeader(chain, headers[index], parent, false, seals[index], unixNow)
}

// VerifyUncles verifies that the given block's uncles conform to the consensus
// rules of the stock Ethereum frkhash engine.
func (frkhash *Frkhash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := frkhash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true, time.Now().Unix()); err != nil {
			return err
		}
	}
	return nil
...
func (frkhash *Frkhash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	expected := frkhash.CalcDifficulty(chain, header.Time, parent)
	...
	if seal {
		if err := frkhash.verifySeal(chain, header, false); err != nil {
			return err
		}
	}
...
// either using the usual frkhash cache for it, or alternatively using a full DAG
...
func (frkhash *Frkhash) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, fulldag bool) error {
	...
	digest, result = frankomoto(frkhash.SealHash(header).Bytes(), header.Nonce.Uint64())
...
// header to conform to the frkhash protocol. The changes are done inline.
func (frkhash *Frkhash) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	parent := chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
	if parent == nil {
		return consensus.ErrUnknownAncestor
	}
	header.Difficulty = frkhash.CalcDifficulty(chain, header.Time, parent)
	return nil
}
...
func (frkhash *Frkhash) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// Finalize block
	frkhash.Finalize(chain, header, state, txs, uncles)
